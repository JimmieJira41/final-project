  ...
  <div class="container">
    ...
    <div class="card borderless shadow p-3">
      ...
      <form class="form-group">
        <!-- <h3>ข้อมูลรายการสินค้า</h3>
      <div class="btn btn-primary" v-on:click="addMultiStock()">เพิ่มช่องสต็อก</div> -->
        <div class="row p-3">
          ...
          <div class="col-12 col-sm-12 col-md-6 col-lg-3 col-xl-3">
            <label for="total_use">จำนวนสินค้าจากสต็อก</label>
            <input
              v-model="total_use"
              type="number"
              class="form-control text-center"
      ...
      <div class="row">
        <div class="col-lg-12 col-md-12 col-sm-12 col-12 my-3 text-end">
          <button class="btn btn-primary mx-1" v-on:click="submitCreateitem()">
            ยืนยัน
          </button>
...
export default {
  ...
  methods: {
    log() {
      console.log("stock list : " + this.id_stock);
    },
    addMultiStock() {
      if (!this.multi_stock) {
    ...
    submitCreateitem() {
      ...
      this.$swal({
        title: "แจ้งเตือน",
        text: "คุณต้องการสร้างรายการสินค้าใหม่ ใช่หรือไม่ ?",
        icon: "warning",
        ...
        if (result.isConfirmed) {
          axios
            .post("/api/item/new-item", itemObj, this.headers)
            .then((response) => {
