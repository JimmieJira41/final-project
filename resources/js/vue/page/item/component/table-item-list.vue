      ...
      <table>
        ...
        <tbody v-if="isTableReady">
          <tr v-for="(item, index) in itemList" v-bind:key="index">
            ...
            <td class="text-center" v-on:click="viewDetailItem(item.id_item)">
              <i class="fas fa-eye"></i>
            </td>
            <td class="text-center">
              <router-link :to="'/item/update-item/' + item.id_item"
                ><i class="fas fa-cog"></i
              ></router-link>
            </td>
            <td class="text-center" v-on:click="deleteItem(item.id_item)">
...
export default {
  ...
  methods: {
    ...
    viewDetailItem(itemId) {
      axios
        .get("/api/item/search-item/" + itemId, this.headers)
        .then((response) => {
    ...
    deleteItem(itemId) {
      this.$swal({
        title: "แจ้งเตือน",
        text: "คุณต้องการลบรายการสินค้า ใช่หรือไม่ ?",
        icon: "warning",
